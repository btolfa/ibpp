...
void StatementImpl::Prepare(const std::string& sql)
{
	...
	Close();
	(mDriver->m_dsql_allocate_statement)(status.Self(), mDatabase->GetHandlePtr(), &mHandle);
	if (status.Errors())
	...
	*/
	...
	else if (mOutRow->Columns() > mOutRow->AllocatedSize())
	{
		...
		*/
		...
		status.Reset();
		(mDriver->m_dsql_describe)(status.Self(), &mHandle, 1, mOutRow->Self());
		if (status.Errors())
		{
			Close();
			throw SQLExceptionImpl(status, "Statement::Prepare",
	...
	if (inEstimate > 0)
	{
		...
		status.Reset();
		(mDriver->m_dsql_describe_bind)(status.Self(), &mHandle, 1, mInRow->Self());
		if (status.Errors())
		{
			Close();
			throw SQLExceptionImpl(status, "Statement::Prepare",
...
void StatementImpl::Execute(const std::string& sql)
{
	...
	else
	{
		...
		(mDriver->m_dsql_execute2)(status.Self(), mTransaction->GetHandlePtr(),
			&mHandle, 1, mInRow == 0 ? 0 : mInRow->Self(),
...
void StatementImpl::CursorExecute(const std::string& cursor, const std::string& sql)
{
	...
	status.Reset();
	(mDriver->m_dsql_set_cursor_name)(status.Self(), &mHandle, const_cast<char*>(cursor.c_str()), 0);
	if (status.Errors())
	{
		//Close();	Commented because Execute error should not free the statement
